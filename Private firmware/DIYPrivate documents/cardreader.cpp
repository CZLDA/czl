...
#if ENABLED(SDSUPPORT) && ENABLED(POWEROFF_SAVE_SD_FILE)     //断电续打加入
...
void CardReader::checkautostart(bool force) {
  ...
  while (root.readDir(p, NULL) > 0) {
    ...
    if (p.name[9] != '~' && strncmp((char*)p.name, autoname, 5) == 0) {
      ...
      enqueuecommand(cmd);               //断电续打  enquecommand(cmd);
      enqueuecommands_P(PSTR("M24"));    //enquecommand_P(PSTR("M24"));
      found = true;
    }
  }
  if (!found)
...
void CardReader::printingHasFinished() {
  ...
  if (file_subcall_ctr > 0) { // Heading up to a parent file that called current as a procedure.
    ...
    file.close();
    sdprinting = false;
    #if ENABLED(SDSUPPORT) && ENABLED(POWEROFF_SAVE_SD_FILE)        //如果启用（SD支持）&启用（断电保存SD文件） 
    ...
    autotempShutdown();
	enqueuecommands_P(PSTR("M81"));		// Printing has Finished power off machine 打印完成了断电机
  }
}
