...
#define HEATER_3_PIN -1    //加热器
#define TEMP_3_PIN -1      //温度

#if MB(GEN7_CUSTOM)     //mb(基因7_自定义)
  ...
  #include "pins_99.h"
#else
  #error Unknown MOTHERBOARD value set in Configuration.h    //配置中设置的未知主板值。
#endif

// List of pins which to ignore when asked to change by gcode, 0 and 1 are RX and TX, do not mess with those!
//当被要求更改gcode时，0和1是rx和tx，请不要混淆它们
#define _E0_PINS E0_STEP_PIN, E0_DIR_PIN, E0_ENABLE_PIN, HEATER_0_PIN, analogInputToDigitalPin(TEMP_0_PIN),
//E0引脚  E0关闭引脚  E0 DIR引脚  E0启用引脚   E0加热器引脚   模拟数字（温度0针）
#define _E1_PINS 
#define _E2_PINS
#define _E3_PINS
  ...
  #define _E1_PINS E1_STEP_PIN, E1_DIR_PIN, E1_ENABLE_PIN, HEATER_1_PIN, analogInputToDigitalPin(TEMP_1_PIN),
    ...
    #endif
  #endif
#elif ENABLED(Y_DUAL_STEPPER_DRIVERS) || ENABLED(Z_DUAL_STEPPER_DRIVERS)  //启用(y双步进驱动程序)||启用(z双步进驱动程序)
  #undef _E1_PINS
  #define _E1_PINS E1_STEP_PIN, E1_DIR_PIN, E1_ENABLE_PIN,
#endif

#ifdef X_STOP_PIN    //x停止引脚
  #if X_HOME_DIR < 0
...
#if ENABLED(DISABLE_MAX_ENDSTOPS)   // 启用(禁用最大端点)
  #undef X_MAX_PIN
  #undef Y_MAX_PIN
  #undef Z_MAX_PIN
  #define X_MAX_PIN          -1
...
#if ENABLED(DISABLE_MIN_ENDSTOPS)   //启用(禁用最小端点)
  #undef X_MIN_PIN
  #undef Y_MIN_PIN
  #undef Z_MIN_PIN
  #define X_MIN_PIN          -1
...
#if ENABLED(DISABLE_XMIN_ENDSTOP)   //启用（禁用xmin终端）
  #undef X_MIN_PIN
  #define X_MIN_PIN          -1
#endif
