...
static void lcd_status_screen();
  ...
  static void lcd_filament_change0();                 //显示菜单
...
static void lcd_main_menu() {
  ...
  if (movesplanned() || IS_SD_PRINTING) {
    ...
    MENU_ITEM(submenu, MSG_PREPARE, lcd_prepare_menu);//准备菜单
	MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);//控制菜单
    #if ENABLED(DELTA_CALIBRATION_MENU)
...
void _lcd_preheat(int endnum, const float temph, const float tempb, const int fan) {
  ...
  static void lcd_preheat_pla_menu() {
    ...
    #endif
    #if TEMP_SENSOR_BED != 0
 //     MENU_ITEM(function, MSG_PREHEAT_PLA_BEDONLY, lcd_preheat_pla_bedonly); // PLA
    #endif
    END_MENU();
  }
  ...
  static void lcd_preheat_abs_menu() {
    ...
    #endif
    #if TEMP_SENSOR_BED != 0
  //    MENU_ITEM(function, MSG_PREHEAT_ABS_BEDONLY, lcd_preheat_abs_bedonly);  // ABS
    #endif
    END_MENU();
  }
...
//	MENU_ITEM(back, "ment to comes out of" , lcd_status_screen);
	MENU_ITEM(back, "\x9f\xc4 ...", lcd_status_screen);
...
void lcd_filament_load1(){ 
	...
	MENU_ITEM(back, "Wait for the fila-", lcd_status_screen);
	MENU_ITEM(back, "ment to comes out of" , lcd_status_screen);
	MENU_ITEM(back, "the extruder2..." , lcd_status_screen);
	END_MENU();
...
//	MENU_ITEM(back, "the extruder1..." , lcd_status_screen);
	...
	enqueuecommands_P((PSTR("G92  E0")));
	enqueuecommands_P((PSTR("M109 S210")));
...
// int i;
 ...
 {
 enqueuecommands_P((PSTR("G1 F200 Z3"))); //Z轴升高3MM
 enqueuecommands_P((PSTR("G1 X150 Y150 F5000")));//X,Y移动到150,150点
 enqueuecommands_P((PSTR("G1 F200 Z-3")));//Z轴下降3mm
 }
 if(i==6)
 {
...
//  MENU_ITEM(submenu, MSG_MOTION, lcd_control_motion_menu);	//电机参数
//  MENU_ITEM(submenu, MSG_VOLUMETRIC, lcd_control_volumetric_menu);	//材料直径
...
void lcd_update() {
  ...
  lcd_buttons_update();
    ...
    if (sd_status != lcd_sd_status && lcd_detected()) {
      lcdDrawUpdate = 2;
      lcd_implementation_init( // to maybe revive the LCD if static electricity killed it.  如果静电击毙了液晶显示器可能会使它复活
        #if ENABLED(LCD_PROGRESS_BAR)
          currentMenu == lcd_status_screen
        #endif
      );
  ...
  if (ms > next_lcd_update_ms) {
      ...
      slow_buttons = lcd_implementation_read_slow_buttons();
	   // buttons which take too long to read in interrupt context 在中断上下文中读取太长的按钮
    #endif
