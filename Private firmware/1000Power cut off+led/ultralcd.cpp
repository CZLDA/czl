...
{
/*
	// If the Z axis is less than 10 millimeters, it goes up 10 millimeters and goes to zero.
...
static void lcd_tune_menu() {
  ...
  // Display and modify the extrusion head temperature.
  MENU_ITEM_EDIT(int3, MSG_NOZZLE, &target_temperature[0], 0, HEATER_0_MAXTEMP - 15);
  // Display and modify hot bed temperature.
...
void _lcd_preheat(int endnum, const float temph, const float tempb, const int fan) {
  ...
  static void lcd_preheat_pla_menu() {
    ...
    #endif
    #if TEMP_SENSOR_BED != 0
      // MENU_ITEM(function, MSG_PREHEAT_PLA_BEDONLY, lcd_preheat_pla_bedonly); // PLA preheat the hot bed
    #endif
    END_MENU();
  }
  ...
  static void lcd_preheat_abs_menu() 
  {
    ...
    #endif
    #if TEMP_SENSOR_BED != 0
      // MENU_ITEM(function, MSG_PREHEAT_ABS_BEDONLY, lcd_preheat_abs_bedonly);  // ABS preheat the hot bed
    #endif
    END_MENU();
  }
...
{ 
	START_MENU();
	MENU_ITEM(back, "Wait for the fila-", lcd_status_screen);
	MENU_ITEM(back, "ment to comes out of" , lcd_status_screen);
	MENU_ITEM(back, "the extruder ..." , lcd_status_screen);
	END_MENU();

	enqueuecommands_P((PSTR("G92  E0")));
	enqueuecommands_P((PSTR("M109 S205")));
	enqueuecommands_P((PSTR("G1 F150 E150")));
	lcd_return_to_status(); 
}

void lcd_filament_unload()
{
	START_MENU();
	MENU_ITEM(back, "Wait for the fila-", lcd_status_screen);
	...
	enqueuecommands_P((PSTR("G92  E0")));
	enqueuecommands_P((PSTR("M109 S205")));
	enqueuecommands_P((PSTR("G1 F150 E-150")));
	lcd_return_to_status(); 
}
...
void next()
{
  ...
  { 
    ...
    enqueuecommands_P((PSTR("G28 X0 Y0 Z0"))); // Home X Y Z axis
    enqueuecommands_P((PSTR("G1 F100 Z3;"))); // Coordinated Movement Z 3mm
    enqueuecommands_P((PSTR("G90"))); // Use Absolute Coordinates
    enqueuecommands_P((PSTR("G1 X100 Y100 F5000"))); // Coordinated Movement X:100mm Y:100mm
    enqueuecommands_P((PSTR("G1 F100 Z-3"))); // Coordinated Movement Z -3mm
    // lcd_return_to_status();
  }
  if(i==2)
  {
    enqueuecommands_P((PSTR("G1 F200 Z3"))); // Coordinated Movement Z 3mm
    enqueuecommands_P((PSTR("G1 Y900 F5000"))); // Coordinated Movement X:100mm Y:900mm
    enqueuecommands_P((PSTR("G1 F200 Z-3"))); // Coordinated Movement Z -3mm
  }
  if(i==3)
  {
    enqueuecommands_P((PSTR("G1 F200 Z3"))); // Coordinated Movement Z 3mm
    enqueuecommands_P((PSTR("G1 X900 F5000"))); // Coordinated Movement X:900mm Y:900mm
    enqueuecommands_P((PSTR("G1 F200 Z-3"))); // Coordinated Movement Z -3mm
  }
  if(i==4)
  {
    enqueuecommands_P((PSTR("G1 F200 Z3"))); // Coordinated Movement Z 3mm
    enqueuecommands_P((PSTR("G1 Y100 F5000"))); // Coordinated Movement X:900mm Y:100mm
    enqueuecommands_P((PSTR("G1 F200 Z-3"))); // Coordinated Movement Z -3mm
  }
  if(i==5)
  {
    enqueuecommands_P((PSTR("G1 F200 Z3"))); // Coordinated Movement Z 3mm
    enqueuecommands_P((PSTR("G1 X500 Y500 F5000"))); // Coordinated Movement X:500mm Y:500mm
    enqueuecommands_P((PSTR("G1 F200 Z-3"))); // Coordinated Movement Z -3mm
  }
  if(i==6)
  {
    enqueuecommands_P((PSTR("G1 F200 Z3"))); // Coordinated Movement Z 3mm
    // enqueuecommands_P((PSTR("G1 X100 Y100 F5000"))); // Coordinated Movement X:150mm Y:150mm
    // enqueuecommands_P((PSTR("G1 F200 Z-3"))); // Coordinated Movement Z -3mm
    i=0;
    lcd_return_to_status();
  }
...
static void lcd_control_menu() 
{
  ...
  MENU_ITEM(submenu, MSG_MOTION, lcd_control_motion_menu);	// enable motion menu
  // MENU_ITEM(submenu, MSG_VOLUMETRIC, lcd_control_volumetric_menu);	// control volumetric menu
...
static void lcd_control_temperature_menu() 
{
  ...
  // Nozzle, Bed, and Fan Control
  // nozzle_bed_fan_menu_items(encoderLine, _lineNr, _drawLineNr, _menuItemNr, wasClicked, itemSelected);
  // Display and modify the nozzles head temperature.
  MENU_ITEM_EDIT(int3, MSG_NOZZLE, &target_temperature[0], 0, HEATER_0_MAXTEMP - 15);
  // Display and modify hot bed temperature.
  MENU_ITEM_EDIT(int3, MSG_BED, &target_temperature_bed, 0, BED_MAXTEMP - 15);
  //
